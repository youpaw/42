...
int						launch_job(t_job *job, int is_foreground,
									int is_forked);
...
void					fork_and_launch_processes(t_job *job,
													int is_foreground);
...
char					get_status_sign_for_job(int job_index,
												int is_job_builtin);
...
void					redirect_init_process_file(t_process *process,
													int from, int to);
int						redirect_print_error(int errcode, const char *token);
int						redirect_parse_right_side(t_token *token,
													int open_options,
													int can_be_number,
													int is_maybe_minus);
int						redirect_parse_left_side(t_ast *leafs,
												int default_value);
...
int						redirect_great(t_ast *leafs, t_process *process,
													int is_double_great);
