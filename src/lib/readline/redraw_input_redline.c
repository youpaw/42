...
static void restore_pos(t_inp *inp, int wide)
{
	...
	x_pos = inp->line_len[inp->curs_y_pos] - (inp->curs_x_pos - (inp->curs_x_pos % wide));
	up = x_pos / wide;
	if (!(inp->line_len[inp->curs_y_pos] % wide))
		up--;
	y_pos = inp->curs_y_pos + 1;
	...
	tputs(tgoto(tgetstr("ch", NULL), 1, inp->curs_x_pos % wide), 1, putchar);
}
...
void redraw_input_readline(t_inp *inp)
{
	...
	if (inp->line_len[inp->curs_y_pos] != inp->curs_x_pos)
	{
		tputs(tgetstr("ce", NULL), 1, &putchar); //clear till the end of curr line
		ioctl(STDIN_FILENO, TIOCGWINSZ, &ws);
		if (inp->len / ws.ws_col != inp->curs_x_pos / ws.ws_col)
		{
